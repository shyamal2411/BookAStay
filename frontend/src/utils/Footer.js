...
const Footer = () => {
    ...
    useEffect(() => {
        const handleScroll = () => {
            ...
            if (documentHeight - scrollTop - windowHeight < footerHeight + 100) {
                setShowFooter(true);
            } else {
                setShowFooter(false);
            }
    ...
    const hideFooter = pathname === "/search" || !showFooter ? "hidden" : "";

    return (
        <footer
            id="footer"
            className={`body-font ${footerClass} ${hideFooter} w-full bg-blue-100 text-gray-600
        `}>
